...
h=a_3;
J=-1.0/m*(sum(sum(y_matrix.*log(h)))+sum(sum((1-y_matrix).*log(1-h))));

%Regularize cost function code should in gerneral work with theta(1) and theta(2) for any size
regularizedTerm=lambda/(2.0*m)*(sum(sum(Theta1(1:end,2:end).^2))+sum(sum(Theta2(1:end,2:end).^2)));
J=J+regularizedTerm;
